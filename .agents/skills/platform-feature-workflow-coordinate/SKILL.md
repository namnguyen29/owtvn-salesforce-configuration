...
- For an enhancement to an existing feature:
  ...
  - Re-run only the validations and tests relevant to the touched areas.
...
- When a selected skill requires deployment validation, do not report completion before attempting the required validation step unless the environment blocks it.
