#!/usr/bin/env python3
"""
Parse Apex test results and format for Codex auto-fix loop.

This hook parses the JSON output from `sf apex run test` and provides
structured feedback that enables Codex to automatically fix failing tests.
...
def format_output(results: dict) -> str:
    """Format test results for Codex consumption."""
    ...
    if failures:
        ...
        for i, failure in enumerate(failures, 1):
            ...
            if analysis['auto_fixable']:
                lines.append("   🤖 AUTO-FIXABLE: Yes - Codex can attempt automatic fix")
