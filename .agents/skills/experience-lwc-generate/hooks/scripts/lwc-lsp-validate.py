...
- Outputs errors to Codex so it can automatically fix them
...
# The installer places lsp-engine at ~/.agents/lsp-engine/ but in a dev
# repo checkout it lives at <repo>/shared/lsp-engine/.  Try both.
SCRIPT_DIR = Path(__file__).parent
PLUGIN_ROOT = SCRIPT_DIR.parent.parent
_LSP_CANDIDATES = [
    Path.home() / ".agents" / "lsp-engine",               # Installed path
...
def format_lwc_diagnostics(
    ...
    """
    Format LSP validation result for Codex hooks.

    This output is designed to be understood by Codex so it can
